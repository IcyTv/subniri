...
impl deluxe::ParseMetaItem for RenameRule {
    fn parse_meta_item(
        ...
        match rule.as_str() {
            ...
            _ => Err(syn::Error::new(span, "Expected a serde-style rename_all rule")).map_err(Into::into),
        }
    }
}
...
impl deluxe::ParseMetaItem for ListStyle {
    fn parse_meta_item(
        ...
        match style.as_str() {
            ...
            "auto" => Ok(Self::Auto),
            _ => Err(syn::Error::new(span, "Expected children, inline, or auto")).map_err(Into::into),
        }
    }
}
...
pub fn generate_config_file_serialize(mut input: DeriveInput) -> Result<TokenStream, deluxe::Error> {
    let opts: ConfigOpts = deluxe::extract_attributes(&mut input)?;
    let name = &input.ident;
    let body = match &mut input.data {
...
fn generate_for_document(opts: &ConfigOpts, s: &mut syn::DataStruct) -> TokenStream {
    ...
    if !opts.children.unwrap_or(true) {
        abort!(s.struct_token, "ConfigFile derives must use children-based parsing");
    }
...
fn parse_child_field(
    ...
    seen_ident: Option<&syn::Ident>,
    _default_expr: Option<&syn::Expr>,
    _is_option: bool,
    key: Option<&str>,
) -> Option<TokenStream> {
    if is_option(ty) {
...
fn list_style_bool(style: Option<ListStyle>, cutoff: Option<usize>, len_ident: &syn::Ident) -> TokenStream {
...
fn is_value_type(ty: &syn::Type) -> bool {
    match ty {
        syn::Type::Path(p) => p
            .path
            .get_ident()
            .map_or(false, |id| KDL_VALUE_TYPES.contains(&id.to_string().as_str())),
        _ => false,
    }
}
...
fn seen_ident(field_ident: &syn::Ident, ty: &syn::Type) -> Option<syn::Ident> {
    ...
    if is_bool(ty) {
        return Some(syn::Ident::new(&format!("__seen_{}", field_ident), field_ident.span()));
    }
    ...
    Some(syn::Ident::new(&format!("__seen_{}", field_ident), field_ident.span()))
}
...
fn is_option(ty: &syn::Type) -> bool {
    match ty {
        syn::Type::Path(p) if p.path.segments.len() == 1 && p.path.segments[0].ident == "Option" => {
            matches!(p.path.segments[0].arguments, syn::PathArguments::AngleBracketed(_))
        }
        _ => false,
    }
}
